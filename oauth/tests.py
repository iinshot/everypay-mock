...
from oauth.models import OAuthClient, ThirdPartyCode, PKCEAuthorizationCode, AccessToken, RefreshToken
...
def test_authorize_get_shows_form(client, oauth_client, third_party_code, bank, account):
...
def test_authorize_get_invalid_redirect_uri(client, oauth_client, third_party_code, bank):
    response = client.get("/oauth/authorize", {
        "client_id": oauth_client.client_id,
...
def test_token_exchange_code_cannot_reuse(client, pkce_auth_code):
    ...
    response = client.post("/oauth/token", data=payload, content_type="application/json")
    assert response.status_code == 400
